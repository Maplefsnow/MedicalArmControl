...
void Axis::changeVelByRate(DOUBLE rate) {
	throwError(Acm_AxChangeVelExByRate(this->hand, rate, 1000, 1000));
}

void Axis::changeVel(DOUBLE newvel) {
	throwError(Acm_AxChangeVel(this->hand, newvel))
}
...
U32 Axis::getMotionStatus() {
	if (this->hand == 0) return 0;

	U32 status = 0;
	throwError(Acm_AxGetMotionStatus(this->hand, &status));
	return status;
}
